...
export default ({ teams }) => {
    ...
    return (
        <AdminLayout>
            <Button onClick={() => router.push('/admin/departments')} variant="light">‹ 뒤로가기</Button><br /><br />
            <h1><b>{(teams.length != 0) ? teams[0].department.dep_name : ''}</b> 부서 팀 관리</h1>
            <p>
                ...
                <tbody>
                    {teams.length != 0 ? teams.map(team => (
                        <tr>
                            ...
                            <td>
                                <Button variant="warning" size="sm" data-team-id={team.team_id} onClick={updateTeam}>편집</Button>{' '}
                                <Button variant="danger" size="sm" data-team-id={team.team_id} onClick={deleteTeam}>삭제</Button>
                            </td>
                        </tr>
                    )) : <tr><td colSpan={4} className="text-center">현재 팀이 없습니다.</td></tr>}
                </tbody>
            </Table>
        </AdminLayout>
    )
...
export async function getServerSideProps(context) {
    const dep_id: number = Number(context.query.dep_id)
